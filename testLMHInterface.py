...
class TestLMHInterface(unittest.TestCase):
    ...
    def test_basic_functionality_isbn(self):
        result = subprocess.run(
            ['python', 'lmh.py', '-i', 'test_data/small_sample_isbns.txt', '-o', 'output.tsv', '--retrieve-ocns'],
    ...
    def test_basic_functionality_oclc(self):
        result = subprocess.run(
            ['python', 'lmh.py', '-i', 'test_data/small_sample_ocns.txt', '-o', 'output.tsv', '--retrieve-isbns'],
            capture_output=True, text=True)
        self.assertEqual(result.returncode, 0)
    ...
    def test_output_file_handling(self):
        result = subprocess.run(['python', 'lmh.py', '-i', 'test_data/small_sample_isbns.txt', '--retrieve-ocns'],
                                capture_output=True, text=True)
        self.assertEqual(result.returncode, 0)
        # self.assertTrue("Retrieved OCNs: ['000000000', '000000000']" in result.stdout)
